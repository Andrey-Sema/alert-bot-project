...
Redis ACL/monitoring credentials описаны в `redis_isolation.md`. Открытые пункты: TLS внутреннего Redis/monitoring, vault/rotation, права файлов на хосте, запуск на провайдере, backup restore и нагрузочная приёмка. Изменение grants после старта будет замечено readiness, но автоматическое прекращение всех текущих задач по такому событию отдельно не реализовано; неподходящая роль не может выполнить запрещённый SQL благодаря самой БД.

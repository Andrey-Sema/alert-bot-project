...
## Progress and supported limits

PR #9 implements N01–N04 and passed its CI run. Stacked PR #10 implements N05–N08 and passed CI run 36 with Redis and PostgreSQL; production acceptance remains pending. The source replay and per-recipient deduplication horizon is 30 days. Source markers created before that policy receive a 30-day TTL from the first successful scraper cleanup pass. A post replayed after the horizon may re-enter the source stream, so end-to-end exactly-once delivery outside that horizon is not claimed. The matcher benchmark budget is under 10 seconds to build 5000 phrases, under 100 ms P95 match time, and under 100 MiB peak allocation in the test workload.

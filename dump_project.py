...
print("=== СТАРТ ОЧИЩЕННОГО УЛЬТРА-ДАМПЕРА (СЕПАРАЦИЯ ЯДРА И ТЕСТОВ) ===", flush=True)
...
for root, dirs, files in os.walk(project_root, topdown=True):
    ...
    dirs[:] = [d for d in dirs if d not in IGNORE_DIRS and not d.startswith(".") and "venv" not in d.lower()]
    ...
    for file in files:
        ...
        file_entry = {"path": relative_path, "content": content}
...
print(f"\n{'=' * 50}", flush=True)
print("🏁 Раздельная генерация дампов завершена успешно!", flush=True)

...
**OdesaAlert Bot** — це допоміжний сервіс, який аналізує текст вибраного Telegram-каналу та надсилає **персоналізовані нічні сповіщення** за збігом категорій і локацій. Можливі хибні сигнали, пропуски й затримки; актуальний стан перевіряйте в офіційних джерелах.

### Ключові особливості

- 🌙 **Нічні сповіщення** — бот надсилає повідомлення за нічним розкладом; гучність визначають налаштування телефону
...
- 📊 **Метрики та операційні алерти** — Prometheus + Grafana + Alertmanager; доставку адміністративних алертів перевіряють на робочому середовищі
...
Telegram-канал (джерело) 
  ...
  │  Streams    │     (XREADGROUP, підтвердження після запису завдань)
  └──────┬──────┘
         │ xreadgroup
         ▼
  ┌─────────────┐
  ...
  └──────┬──────┘     → посторінкова вибірка адресатів із PostgreSQL
         │ стійкі завдання доставки
         ▼
  ┌─────────────┐
  │ Broadcaster │  ← Redis delivery stream + ZSET затримок
...
- **Дедуплікація** — стабільна ідентичність джерела та адресата в Redis із TTL сім днів
- **TextProcessor** — прекомпільовані regex з `\w{0,3}` суфіксом для відмінювання
- **Вибірка адресатів** — обмежені сторінки з PostgreSQL без завантаження всієї аудиторії в пам'ять
...
- **Broadcaster** — 15 паралельних asyncio-воркерів, Redis delivery stream та окремий DLQ
...
| `/delete_me` | Видалення активного профілю та черги сповіщень після підтвердження |
...
Політика даних, вимірювання MAU/ємності та управління секретами: [docs/data_capacity_security.md](docs/data_capacity_security.md).
Операційні аварійні процедури: [docs/oncall_runbook.md](docs/oncall_runbook.md). Модель загроз і стан перевірки: [docs/threat_model_asvs.md](docs/threat_model_asvs.md).
...
- **TLS для PostgreSQL** — перевірка сертифіката та імені сервера системним CA

...
from alert_bot_project.bot.keyboards.builders import (
    ...
    build_sound_picker_keyboard,
    build_threat_categories_keyboard,
)
from alert_bot_project.bot.keyboards.messages import REPEATS_INFO_TEXT, SOUND_PICKER_TEXT, SOUND_SEND_CAPTION
...
DEFAULT_SOUND_PATH = Path(__file__).resolve().parent.parent / "sounds" / "alert_siren.mp3"
CUSTOM_SOUNDS_DIR = Path("/data/custom_sounds")


def _list_custom_sound_names() -> list[str]:
    """Скан теки з кастомними звуками (bind-mount, поза git/образом) на льоту."""
    if not CUSTOM_SOUNDS_DIR.is_dir():
        return []
    return sorted(p.stem for p in CUSTOM_SOUNDS_DIR.glob("*.mp3"))


@router.callback_query(F.data == MENU_CHOOSE_GROUP)
async def show_group_selection(callback: CallbackQuery) -> None:
    await cast(Message, callback.message).edit_text(
        "Оберіть регіональну групу дислокацій для налаштування:", reply_markup=build_group_selection_menu()
    )
    await callback.answer()


@router.callback_query(F.data == MENU_CUSTOM_MANAGE)
async def show_custom_phrases_menu(callback: CallbackQuery, db_session: AsyncSession) -> None:
...
async def show_paginated_locations(
    callback: CallbackQuery, callback_data: GroupNavCallback, db_session: AsyncSession
) -> None:
    assert callback.from_user is not None
    user = await get_or_create_user(db_session, callback.from_user.id)
    await cast(Message, callback.message).edit_text(
...
async def toggle_location_trigger(
    callback: CallbackQuery, callback_data: LocationToggleCallback, db_session: AsyncSession
) -> None:
...
async def show_threat_categories(callback: CallbackQuery, db_session: AsyncSession) -> None:
    assert callback.from_user is not None
    user = await get_or_create_user(db_session, callback.from_user.id)
    await cast(Message, callback.message).edit_text(
...
async def toggle_threat_category(
    callback: CallbackQuery, callback_data: ThreatCategoryCallback, db_session: AsyncSession
) -> None:
...
async def show_mute_options(callback: CallbackQuery) -> None:
    await cast(Message, callback.message).edit_text(
        text="🔕 <b>Режим тиші (MUTE):</b>", reply_markup=build_mute_options_keyboard()
    )
    await callback.answer()
...
async def process_mute_action(
    callback: CallbackQuery, callback_data: MutePresetCallback, db_session: AsyncSession
) -> None:
    assert callback.from_user is not None
    try:
        service = UserService(db_session, redis_client)
...
@router.callback_query(F.data == MENU_SOUNDS)
async def show_sound_picker(callback: CallbackQuery) -> None:
    await cast(Message, callback.message).edit_text(
        text=SOUND_PICKER_TEXT, reply_markup=build_sound_picker_keyboard(_list_custom_sound_names())
    )
    await callback.answer()


@router.callback_query(SoundSelectCallback.filter())
async def send_selected_sound(callback: CallbackQuery, callback_data: SoundSelectCallback) -> None:
    if callback_data.name == DEFAULT_SOUND_NAME:
        sound_path = DEFAULT_SOUND_PATH
    else:
        # Ім'я має точно збігатися з файлом, знайденим у теці ЗАРАЗ — жодного
        # прямого підставлення callback_data у шлях без звірки зі списком.
        if callback_data.name not in _list_custom_sound_names():
            await callback.answer("Цей звук більше не доступний", show_alert=True)
            return
        sound_path = CUSTOM_SOUNDS_DIR / f"{callback_data.name}.mp3"
...
async def initiate_custom_trigger_prompt(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await cast(Message, callback.message).answer(
        "✍ <b>Введіть назву вашої кастомної локації:</b>\n\n"
        "⚠ <b>Важливо:</b> Моніторинг працює виключно по Одесі та Одеській області. "
        "Вводити інші міста України немає сенсу.\n\n"
        "💡 <b>Порада щодо введення:</b> вводьте <b>основу слова</b> без закінчення.\n"
        "Наприклад, замість «Малиновського» введіть <b>малиновск</b>, "
        "замість «Олександрівка» введіть <b>олександрівк</b>.\n"
        "<i>Алгоритм автоматично прорахує до 3 символів закінчення (відмінки, роди, множину) у звітах пабліків!</i>"
    )
    await callback.answer()


@router.message(CustomTriggerStates.waiting_for_keyword)
async def store_custom_user_keyword(message: Message, state: FSMContext, db_session: AsyncSession) -> None:
    if not message.text:
        return
    ...
    for _loc_key, pattern in COMPILED_LOCATIONS.items():
...
async def delete_custom_user_keyword(
    callback: CallbackQuery, callback_data: CustomActionCallback, db_session: AsyncSession
) -> None:
...
async def process_alert_acknowledgement(callback: CallbackQuery, db_session: AsyncSession) -> None:
    assert callback.from_user is not None
    service = UserService(db_session, redis_client)
    await service.acknowledge_alert(callback.from_user.id)

    message = cast(Message, callback.message)
    safe_text = message.html_text or "<b>Загроза</b>"
    await message.edit_text(text=f"{safe_text}\n\n✅ <i>Сигнал прийнято.</i>")

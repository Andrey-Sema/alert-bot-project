...
def create_service_redis(redis_url: str | None = None) -> Redis:
    ...
    if config.SERVICE_ROLE in REDIS_USERS:
        validate_service_redis_url(url, config.SERVICE_ROLE)
...
async def verify_redis_identity(client: Redis, service: str) -> None:
    async with asyncio.timeout(4):
        identity = await client.acl_whoami()
    if identity != REDIS_USERS[service]:
        raise RuntimeError("Unexpected Redis ACL identity")

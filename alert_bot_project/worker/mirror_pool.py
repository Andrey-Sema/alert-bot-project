...
class MirrorPool:
    def __init__(self, tokens: list[str], per_token_rps: float = 25.0, workers_per_token: int = 2):
        ...
        self.queues: list[asyncio.Queue[tuple[int, str, dict[str, Any]]]] = [
            asyncio.Queue(maxsize=10000) for _ in tokens
        ]
    ...
    def send(self, user_id: int, text: str, assigned_idx: int | None = None, **kw: Any) -> None:
    ...
    async def _worker(self, idx: int, bot: Bot) -> None:
        q = self.queues[idx]
        while True:
    ...
    async def _send_one(self, bot: Bot, user_id: int, text: str, kw: dict[str, Any]) -> None:
        try:
